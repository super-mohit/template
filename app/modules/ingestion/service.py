...
from app.models import BusinessObject, ExtractionConfiguration, Job
...
def process_and_save_file(db: Session, file_content: bytes, file_mime_type: str):
    ...
    extracted_data = extractor.extract_data_from_file(
        file_content, file_mime_type, prompt
    )
    ...
    log.info(
        f"Successfully ingested BusinessObject ID: {new_object.id}. Triggering AI analysis..."
    )
