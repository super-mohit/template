...
function log(message: string, data?: object) {
  if (isAuthDebug) {
    console.log(`[AUTH_DEBUG | Middleware] ${new Date().toISOString()}: ${message}`, data || '');
  }
}
  ...
  },
  {
    callbacks: {
      authorized: ({ token, req }) => {
        ...
        log(`Authorization check for path: ${req.nextUrl.pathname}. User is authorized: ${isAuthorized}`);
        if (!isAuthorized) {
