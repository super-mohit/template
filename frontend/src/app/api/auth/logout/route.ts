...
export async function GET(request: NextRequest) {
  try {
    ...
    const appBaseUrl = process.env.NEXTAUTH_URL?.replace('/api/auth', '') || `http://localhost:3001${basePath}`
    ...
    keycloakLogoutUrl.searchParams.set('post_logout_redirect_uri', nextAuthSignoutUrl)
    ...
    if (idToken) {
      ...
      log('⚠️  No id_token available in session. Logout may still work but could be less reliable.')
    }
